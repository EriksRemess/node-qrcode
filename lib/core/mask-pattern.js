  ...
  switch (pattern) {
    case Patterns.PATTERN000:
      for (let row = 0; row < size; row++) {
        ...
        for (let col = 0; col < size; col++) {
          ...
          if (!reserved[index] && ((row + col) & 1) === 0) modules[index] ^= 1
        }
      }
      return
    ...
    case Patterns.PATTERN001:
      for (let row = 0; row < size; row++) {
        ...
        for (let col = 0; col < size; col++) {
          const index = rowOffset + col
          if (!reserved[index]) modules[index] ^= 1
        }
      }
      return
    ...
    case Patterns.PATTERN010:
      for (let row = 0; row < size; row++) {
        ...
        for (let col = 0; col < size; col++) {
          const index = rowOffset + col
          if (!reserved[index] && col % 3 === 0) modules[index] ^= 1
        }
      }
      return
    ...
    case Patterns.PATTERN011:
      for (let row = 0; row < size; row++) {
        ...
        for (let col = 0; col < size; col++) {
          const index = rowOffset + col
          if (!reserved[index] && (row + col) % 3 === 0) modules[index] ^= 1
        }
      }
      return
    ...
    case Patterns.PATTERN100:
      for (let row = 0; row < size; row++) {
        ...
        for (let col = 0; col < size; col++) {
          const index = rowOffset + col
          if (!reserved[index] && ((rowDiv2 + ((col / 3) | 0)) & 1) === 0) modules[index] ^= 1
        }
      }
      return
    ...
    case Patterns.PATTERN101:
      for (let row = 0; row < size; row++) {
        ...
        for (let col = 0; col < size; col++) {
          const index = rowOffset + col
          const mul = row * col
          if (!reserved[index] && (mul & 1) + (mul % 3) === 0) modules[index] ^= 1
        }
      }
      return
    ...
    case Patterns.PATTERN110:
      for (let row = 0; row < size; row++) {
        ...
        for (let col = 0; col < size; col++) {
          const index = rowOffset + col
          const mul = row * col
          if (!reserved[index] && (((mul & 1) + (mul % 3)) & 1) === 0) modules[index] ^= 1
        }
      }
      return
    ...
    case Patterns.PATTERN111:
      for (let row = 0; row < size; row++) {
        ...
        for (let col = 0; col < size; col++) {
          const index = rowOffset + col
          const mul = row * col
          if (!reserved[index] && ((mul % 3 + ((row + col) & 1)) & 1) === 0) modules[index] ^= 1
        }
      }
      return
