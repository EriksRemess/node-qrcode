 ...
 * @return {Array}          Array of object with segments data
 */
function getSegmentsFromString (dataStr, toSJISFunc) {
 ...
 */
function buildSingleSegment (data, modesHint, toSJISFunc) {
  let mode
  ...
  switch (mode) {
    ...
    case Mode.KANJI:
      return new KanjiData(data, toSJISFunc)
 ...
 */
export function fromArray(array, toSJISFunc) {
  const acc = []
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (typeof seg === 'string') {
      acc.push(buildSingleSegment(seg, null, toSJISFunc))
    } else if (seg.data) {
      acc.push(buildSingleSegment(seg.data, seg.mode, toSJISFunc))
    }
  }
 ...
 * @return {Array}          Array of segments
 */
export function fromString(data, version, toSJISFunc) {
  const segs = getSegmentsFromString(data, toSJISFunc)

  const nodes = buildNodes(segs)
  const optimizedSegs = findOptimizedNodes(nodes, version)

  return fromArray(mergeSegments(optimizedSegs), toSJISFunc)
}
 ...
 * @return {Array}       Array of segments
 */
export function rawSplit(data, toSJISFunc) {
  return fromArray(
    getSegmentsFromString(data, toSJISFunc), toSJISFunc
  )
}
